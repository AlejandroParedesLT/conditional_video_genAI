...
def sample(
    ...
    num_frames: Optional[int] = 25,  # 21 for SV3D
    ...
    for input_img_path in all_img_paths:
        ...
        with torch.no_grad():
            with torch.autocast(device):
                ...
                # Collecting frames into a list
                frames = []
                for i, sample in enumerate(samples):
                    # Normalize and clamp the image
                    sample_img = (sample + 1.0) / 2.0  # Normalize to [0, 1]
                    sample_img = torch.clamp(sample_img, 0.0, 1.0)  # Ensure pixel values are in valid range
                    
                    # Convert to HWC format and to uint8
                    img = sample_img.permute(1, 2, 0).cpu().numpy()  # Convert to HWC format
                    img = (img * 255).astype(np.uint8)  # Scale to [0, 255] and convert to uint8
                    
                    # Add to frames list
                    frames.append(img)
                    imageio.imwrite(os.path.join(output_folder, f"{base_count + i:06d}_noSR.jpg"), img)

                # Save all frames as a video
                #video_path = os.path.join(output_folder, f"{base_count:06d}.mp4")
                #imageio.mimwrite(video_path, frames, fps=30)  # Adjust fps if necessary
                ...
                # Collecting frames into a list
                frames = []
                for i, sample in enumerate(vid):
                    frames.append(sample)
                    imageio.imwrite(os.path.join(output_folder, f"{base_count + i:06d}.jpg"), sample)
                """
                video_path = os.path.join(output_folder, f"{base_count:06d}.mp4")
                imageio.mimwrite(uri=video_path, ims=vid, format='mp4')
                print("Video Exported")
                """
                # Determine the dimensions from the first frame
                first_frame = vid[0]  # Make sure vid supports indexing or retrieve the first frame accordingly
                height, width, _ = first_frame.shape

                # Set up the video writer
                video_path = os.path.join(output_folder, f"{base_count:06d}.mp4")
                fourcc = cv2.VideoWriter_fourcc(*'mp4v')
                fps = 8.0  # Adjust the frames per second as needed
                video_writer = cv2.VideoWriter(video_path, fourcc, fps, (width, height))

                # Write each frame directly to the video
                for frame in vid:
                    # If your frame is in RGB format, convert it to BGR as OpenCV expects BGR.
